...
### functions
source("code/R/image_funs.R")

### directories
indir <- "data/calibrations/"

classes <- "123"
burnin <- 2000
iter <- 18000
date <- "2023-09-05"
subDir <- paste0(indir,"bt_run_",date,"_class",classes,"_",burnin,"burnin_",iter,"iter")
pv <- "mprua65d"
...
test <- readRDS(paste0(subDir,"/",pv,"/model_evaluation_test_",pv,".rds"))
train <- readRDS(paste0(subDir,"/",pv,"/model_evaluation_train_",pv,".rds"))
stn_list <- readRDS(ff)
