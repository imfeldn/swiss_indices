...
### functions
source("code/R/helpfuns.R")

### read station list
station_list <- read.csv(paste0("data/pheno_net/data/ch.meteoschweiz.messnetz-phaenologie_de.csv"),header = T, sep = ",", allowEscapes = T)
row.names(station_list) <- as.character(station_list$Abk.)

### read all data
pheno <- read.table(paste0("data/pheno_net/phaeno_previous.csv"),header=T, sep =";")

### read classification
classification <- read.table(paste0("data/pheno_net/dataD4.csv"),header=T, sep =",")

### read qc
qc <- read.csv(paste0("data/pheno_net/PhenoClass/QCflags_minimum_Final_Juni2017.csv"),
               header=T, sep =",")

### select stations based on classes
phenovals <- "maesh13d"#c("mfags13d", "mprua65d")#,"mcora65d","mcora13d","manen65d","mtaro65d","mtusf65d","mcarp65d","mlard13d")
param_id <- 601 #c(606,662)#,666,664,619,617,654,656,653,658,634)
...
for(pv in phenovals){
  ...
  # jsut get the long series
  longseries <- sapply(vval_stns, function(x){nrow(pheno_sel[pheno_sel$nat_abbr == x,])}) >= 35
  vval_stns <- vval_stns[longseries]
  pheno_sel <-  pheno_sel[as.character(pheno_sel$nat_abbr) %in% vval_stns,]
  ...
  saveRDS(
    station_list_sel,
    file = paste0("data-raw/pheno_net/stations_",pv,"_class",cllps(classes),".rds"),
    compress = "xz"
  )
  ...
  saveRDS(
    pheno_dat,
    file = paste0("data-raw/pheno_net/pheno_dat_",pv,"_class",cllps(classes),".rds"),
    compress = "xz"
  )
