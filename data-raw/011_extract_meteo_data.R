...
phenovals <- "maesh13d"#c( "mprua65d","mfags13d")
pv <- phenovals[1]
...
for(pv in phenovals){
  ...
  station_list_sel <- readRDS(paste0("data-raw/pheno_net/stations_",pv,"_class",classes,".rds"))
  ...
  saveRDS(
    meteo_dat,
    file = paste0("data-raw/pheno_net/meteo_",pv,"_class",classes,".rds"),
    compress = "xz"
  )
}
