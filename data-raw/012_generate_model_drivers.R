...
phenovals <- "maesh13d"#c("mfags13d","mprua65d")
classes <- "123"
...
for(pv in phenovals){
  ...
  meteo_file <- file.path("data-raw/pheno_net/",
                          paste0("meteo_",pv,"_class",classes,".rds")
  )
  phenology_file <- file.path("data-raw/pheno_net/",
  ...
  # construct filename
  filename <- file.path(output_dir, paste0(pv, "_full_model_drivers_class",classes,"35ylong.rds"))
